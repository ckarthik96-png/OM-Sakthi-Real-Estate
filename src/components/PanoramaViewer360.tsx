  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (!isUserInteracting.current) {
        // Slow auto rotation oscillation between -12 and +12 degrees for 30° focal sweep
        lon.current += 0.05 * (isUserInteracting.current ? 0 : 1);
        if (lon.current > 14) lon.current = -14;
      }

      // Clamp horizontal angle to 30° sweep (-15° to +15°) and vertical angle (-20° to +20°)
      lon.current = Math.max(-15, Math.min(15, lon.current));
      lat.current = Math.max(-20, Math.min(20, lat.current));
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="font-extrabold text-sm flex items-center gap-1.5">
              WebGL 3D Full-House 30° Precision Focal Tour <Sparkles className="w-3.5 h-3.5 text-[#D4AF37]" />
            </h4>
            <p className="text-[11px] text-slate-400">Click & Drag for 30° high-precision architectural focal inspection</p>
          </div>
        </div>
      ...
      <div className="relative w-full h-[450px] sm:h-[550px] bg-black">
        ...
        {!isLoaded && (
          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center text-white z-10">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300">Rendering WebGL 3D 30° Environment...</span>
          </div>
        )}
        ...
        <div className="absolute top-4 left-4 bg-slate-900/80 backdrop-blur-md px-3.5 py-1.5 rounded-xl border border-white/20 text-[#D4AF37] text-xs font-bold flex items-center gap-2 pointer-events-none z-10">
          ...
          <span>30° Precision Full House Sweep — Drag Mouse / Touch</span>
        </div>
